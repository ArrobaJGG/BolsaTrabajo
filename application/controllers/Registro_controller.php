...
class Registro_controller extends CI_Controller {
	...
	public function cambiar_contrasena_alumno($hash = false) {
		...
		if ($this -> session -> userdata('tipo')=='cambio'&&$this->input->post('enviar')) {
			if($this->alumno_model->existe($this->session->userdata('id_login'))) redirect('../../login_controller');
			...
			if ($correo = $this -> login_model -> existe_correo($hash)) {
				...
				if($this->alumno_model->existe($this->login_model->get_id($correo))) redirect('../../login_controller');
				if ($this -> login_model -> validar_login($correo)) {
					$datos_sesion = array(
						'usuario' => $correo,
			...
			if ($this -> session -> userdata['rol'] == 'administrador') {
				...
				if ($this -> input -> post('usuario')) {
					...
					if ($this -> form_validation -> run() != false) {
						...
						if (!$this -> login_model -> get_correo($this -> input -> post('usuario'))) {
							echo "correo enviado";
							$datos = array('correo' => $this -> input -> post('usuario'), 'rol' => 'alumno', 'contrasena' => password_hash($this -> string_aleatorio(10), PASSWORD_DEFAULT), 'fecha_creacion' => date("Y/m/d"), 'ultimo_login' => date("Y/m/d"), 'validado' => false, 'hash_validar' => str_replace($caracteres_invalidos, '', password_hash(time() . $this -> input -> post('usuario'), PASSWORD_DEFAULT)));
							if ($this -> login_model -> crear_usuario($datos)) {
								//Montar el mail y enviar
								//*
								$this -> email -> from('arrobajgg@gmail.com', 'BolsaTrabajoFPTxurdinaga');
